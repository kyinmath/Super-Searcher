...
{ 
  { "integer", 0, 1 }, 
  { "Hello World!", 0, 0 }, 
  { "if", 3, -1 }, //test, first branch, fields[0] branch. passes through the return object of \
each branch; the return objects must be the same. 
...
previous basic block element. 
  ...
  AST(const char name[], AST* preceding = nullptr, int_or_ptr<AST> f1 = nullptr, int_or_ptr<AST> \
