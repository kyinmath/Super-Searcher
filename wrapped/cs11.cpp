...
class compiler_object
{
  ...
  //these are the labels which we just passed by. you can jump to them without checking finiteness, 
  //but you still have to check the type stack
...
#define finish(X, type) do { llvm::Value* end_value = stack_degree >= 1 ? move_to_stack(X) : X; \
return finish_internal(end_value, type, 0, -1ull); } while (0)
#define finish_pointer(X, type, u, l) do { if (stack_degree >= 1) move_to_stack(X); return \
finish_internal(X, type, u, l); } while (0)
