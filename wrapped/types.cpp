...
so: 0 = different. 1 = new reference is too large. 2 = existing reference is too large. 3 = \
perfect match. 
*/ 
...
space-consuming thing to easily compare. 
  //however, it does NOT acquire or process the object that we're left at. so if there's a TU, \
then the locks have not been erased 
...
reference to accept pieces, but I don't know if that's the best. 
  ...
  //TODO: else, it's not fully immut. remember: some things are naturally immut, like fixed \
uint64. in that case, check fully_immut, but the lock state doesn't matter 


  else 
  { 
    //for pointers, we cannot pass in fully_immut. because the old reference must be valid too, \
and we can't point to anything more general than the old reference. 
