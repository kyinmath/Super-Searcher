...
this checks if types are equivalent, but with many special cases depensing on the nature of the two 
types.


we can only convert things that are FULLY IMMUT. just immut is not enough, because it might change 
anyway (perhaps it's inside a lock, or TU).
...
so if the first pointer says the lock is immut, then we can treat it as fully immut. because it is 
immut for the duration of the fields[0] pointer's life.
fully immut still requires the actual lock to also be immut, before you can start converting.

we have two problems: 1. we might be interested in offsets. solution: you use another function to 
acquire the appropriate subtype.

full pointers have to match the entire type.
thus, we return: 0 if inequal for sure, 1 if fields[0] points to a valid subtype, and 2 if 
fields[0] points to a valid conversion that fills the entire type
if version = reference, it checks if the first type can be referenced by the fields[0].
*/
/*if version == RVO: checks if an object of the first type can be placed in a memory slot of the 
fields[0] type. that means that the fields[0] type is what everyone will refer to it as.
don't worry about lock states of integers or pointers; the target will absorb the lock state 
anyway.
example: an immut TU of pointer, 0, that's been fixed to a pointer, can be converted into just a 
pointer type.

with RVO, first must be smaller than fields[0]. with reference, it's the opposite.
so: 0 = different. 1 = new reference is too large. 2 = existing reference is too large. 3 = perfect 
match.
*/
...
unsigned type_check(type_status version, Type* existing_reference, Type* new_reference)
{
  ...
  //store it twice. but we do anyway, for now.
...
check_next_token:
  //acquire items until we reach a physical token. this makes sure that we're left at a valid, 
  //space-consuming thing to easily compare.
  //however, it does NOT acquire or process the object that we're left at. so if there's a TU, then 
  //the locks have not been erased
  ...
  */
  if (version == RVO)
  {
    ...
    case Typen("fixed integer"): //no need for lock check.
      ...
      return 0;
    case Typen("integer"):
        if (iter[0]->tag == iter[1]->tag || iter[0]->tag == Typen("fixed integer") || iter[0]->tag 
        == Typen("cheap pointer")) //also full pointer
          goto finished_checking;
  ...
  //TODO: else, it's not fully immut. remember: some things are naturally immut, like fixed uint64. 
  //in that case, check fully_immut, but the lock state doesn't matter


  else
  {
    //for pointers, we cannot pass in fully_immut. because the old reference must be valid too, and 
    //we can't point to anything more general than the old reference.
