...
//might_be_end_in_BB is to check if you may be starting a new basic block, which really means that 
//your AST is a field in another AST.
//be careful: an AST and its previous basic block elements may need braces when referenced one way, 
//but may not need them when referenced another way (for example, if it's referenced as 
//dependencies of two different ASTs, but is not the primary element in the first). this isn't an 
//issue when reading, but is handled appropriately when outputting.
struct output_AST_console_version
{
  std::unordered_set<AST*> AST_list = { nullptr }; //nullptr = 0 is a special value.
  std::unordered_set<AST*> reference_necessary; //list of ASTs that have references to them later. 
  //these ASTs need to print a name.
  ...
  output_AST_console_version(AST* target)
  {
    ...
    output_console(target, false); //we call it with "false", because the overall function has no 
    //braces.
  ...
  void determine_references(AST* target)
  {
    ...
    if (target->preceding_BB_element != nullptr) 
    determine_references(target->preceding_BB_element);
    for (int x = 0; x < AST_descriptor[target->tag].pointer_fields; ++x) 
    determine_references(target->fields[x].ptr);
  }
  ...
  void output_console(AST* target, bool might_be_end_in_BB)
  {
    ...
    for (unsigned check_further_nonzero_fields = x + 1; check_further_nonzero_fields < 
    max_fields_in_AST; ++check_further_nonzero_fields)
      if (target->fields[check_further_nonzero_fields].num) final_nonzero_field = 
      check_further_nonzero_fields;
