/*
This creates a new unique type. 
*/
...
std::unordered_set<Type_wrapper_pointer> k; //a hash table of all the unique types.
...
std::pair<Type*, bool> get_unique_type_internal(Type* model)
{
	...
	bool create_new_for_sure; //if this is true, then we don't have to check if this object is in the hash table. we know it is new.
